...
var _ = Describe("Client", func() {
	...
	It("creates a new stream", func(ctx SpecContext) {
		...
		go fakeRabbitMQ.fakeRabbitMQDeclareStream(newContextWithResponseCode(itCtx, 0x0001), "test-stream", costants.StreamConfiguration{"some-key": "some-value"})

		err = streamClient.DeclareStream(itCtx, "test-stream", costants.StreamConfiguration{"some-key": "some-value"})
